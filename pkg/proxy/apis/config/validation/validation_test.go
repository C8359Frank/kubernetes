...
func TestValidateKubeProxyConfiguration(t *testing.T) {
	...
	successCases := []kubeproxyconfig.KubeProxyConfiguration{
		...
		{
			BindAddress:        []string{"192.168.59.103", "fd12:3456:789a:1::1"},
			HealthzBindAddress: "",
			MetricsBindAddress: "127.0.0.1:10249",
			ClusterCIDR:        []string{"192.168.59.0/24", "::/0"},
			UDPIdleTimeout:     metav1.Duration{Duration: 1 * time.Second},
			ConfigSyncPeriod:   metav1.Duration{Duration: 1 * time.Second},
			IPTables: kubeproxyconfig.KubeProxyIPTablesConfiguration{
				MasqueradeAll: true,
	...
	errorCases := []struct {
	...
	}{
		{
			config: kubeproxyconfig.KubeProxyConfiguration{
				...
				BindAddress:        []string{"10.10.12.11:2000"},
				HealthzBindAddress: "0.0.0.0:10256",
				MetricsBindAddress: "127.0.0.1:10249",
				ClusterCIDR:        []string{"192.168.59.0/24"},
				UDPIdleTimeout:     metav1.Duration{Duration: 1 * time.Second},
		...
		{
			config: kubeproxyconfig.KubeProxyConfiguration{
				BindAddress: []string{"10.10.12.11"},
		...
		},
		{
			config: kubeproxyconfig.KubeProxyConfiguration{
				...
				ClusterCIDR:      []string{"192.168.59.0"},
				UDPIdleTimeout:   metav1.Duration{Duration: 1 * time.Second},
				ConfigSyncPeriod: metav1.Duration{Duration: 1 * time.Second},
				IPTables: kubeproxyconfig.KubeProxyIPTablesConfiguration{
					MasqueradeAll: true,
