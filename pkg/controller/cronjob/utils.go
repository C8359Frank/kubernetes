...
// Returns the latest start time in the time window (inclusive), and the number of schedules in the time window (capped at 2).
...
// getLatestMissedSchedule returns the latest start time in the specified time window (inclusive), and a true/false indicating if a start time was found.
...
func getLatestMissedScheduleBinarySearch(startWindow time.Time, endWindow time.Time, schedule cron.Schedule) (time.Time, bool) {
	...
	if !nextScheduleAfterMidway.After(endWindow) {
		...
		if !nextScheduleInWindow.After(midway) { // Latest start time is in first half. There's no start time in the second half.
			latestFound, found := getLatestMissedScheduleBinarySearch(startWindow, midway, schedule) // Check the first half.
			if found {
				return latestFound, true
			} else {
